function get_dijkstra_path(grid, start, stop, dim_h, dim_w){
    var parents = [];
    ...
    while(stack.length!=0){
        ...
        if(u[2][1]-1>=0 && (grid[u[2][1]-1][u[2][0]]==0 || grid[u[2][1]-1][u[2][0]]==3)){
            stack.push([u[0]+1,count, [u[2][0], u[2][1]-1], u[2]])
            count+=1
        }
        ...
        if(u[2][0]-1>=0 && (grid[u[2][1]][u[2][0]-1]==0 || grid[u[2][1]][u[2][0]-1]==3)){
            stack.push([u[0]+1,count, [u[2][0]-1, u[2][1]], u[2]])
            count+=1
        }
...
onmessage = function(event){
    ...
    get_dijkstra_path(event.data[0], event.data[1], event.data[2], event.data[3], event.data[4]);
}